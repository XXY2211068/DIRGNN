...
class DGNN(Module):
    ...
    def compute_scores(self, seq_hidden_local, seq_hidden_global, mask, target_emb, A_hat, D_hat, inputs):
        relation_emb = self.RelationGraph(self.embedding.weight, inputs, A_hat, D_hat, target_emb).unsqueeze(1).expand_as(seq_hidden_local)



        hidden_local = self.self_attention(q=seq_hidden_local,k=relation_emb,v=relation_emb)
        hidden_global = seq_hidden_global
        hidden = self.global_fuse_linear(torch.cat([hidden_local, hidden_global], -1))
    ...
    def forward(self, A_global, inputs_global, inputs_global_index, alias_inputs):
        ...
        hidden_local = hidden_last_global[inputs_global_index]
        get = lambda i: hidden_local[i][alias_inputs[i]]
        seq_hidden_local = torch.stack([get(i) for i in torch.arange(len(alias_inputs)).long()])  # [b,s,2d]


        #全局
        hidden_att_global = self.global_att_blocks(hidden_global)
        hidden_global = hidden_att_global[inputs_global_index]
        get = lambda i: hidden_global[i][alias_inputs[i]]
        seq_hidden_global = torch.stack([get(i) for i in torch.arange(len(alias_inputs)).long()])  # [b,s,2d]
        zeros = trans_to_cuda(torch.Tensor(seq_hidden_global.shape[0], 1, self.hidden_size).fill_(0).float())  # [b,1,d]
        session_target = torch.cat([seq_hidden_global, zeros], 1)
        ...
        zeros = trans_to_cuda(torch.Tensor(seq_hidden_gnn.shape[0], 1, self.hidden_size).fill_(0).float())  # [b,1,d]
        session_target = torch.cat([seq_hidden_gnn, zeros], 1)
        '''
        return seq_hidden_local, seq_hidden_global, target_emb
...
def forward(model, i, data):
    ...
    seq_hidden_local, seq_hidden_global, target_emb = model(A_global, inputs, global_inputs_index, alias_inputs)
    scores = model.compute_scores(seq_hidden_local, seq_hidden_global, mask, target_emb,A_hat, D_hat,items)
