...
parser.add_argument('--dataset', default='Tmall',
                    help='dataset name: diginetica/yoochoose1_4/yoochoose1_64/Gowalla/Nowplaying/Tmall/RetailRocket/sample')
...
parser.add_argument('--fuse_A', action='store_true', default=False,
                    help='whether to fuse an auxiliary adjacent matrix via correlation or self-attention')
parser.add_argument('--global_att_block_nums', type=int, default=5, help='the number of global attention blocks')
parser.add_argument('--global_att_head_nums', type=int, default=4,
                    help='the number of multi-heads for global attention')
...
logging.basicConfig(level=logging.INFO,
                    filename=opt.log_file + opt.dataset + '/' + opt.dataset + '-' + time.strftime("%Y-%m-%d_%H-%M-%S",
                                                                                                  time.localtime()) + '.log',
                    datefmt='%Y/%m/%d %H:%M:%S',
...
def lens_max():
    dict_user_count = {'diginetica': 70, 'RetailRocket': 286,  'yoochoose1_4': 200, 'yoochoose1_64': 146, 'Gowalla': 20, 'LastFM': 20, 'Nowplaying': 29, 'Tmall': 39, 'samples': 17}
